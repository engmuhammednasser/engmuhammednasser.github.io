...
const outputDirectory = path.join(root, "projects", "arabic-window", "full-page");
const edgePath = "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe";
const port = 9331;
const profileDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "arabic-window-capture-"));

const pages = [
  ["01-home", "https://arabicwindow.com/"],
  ["02-courses", "https://arabicwindow.com/courses/"],
  ["03-books", "https://arabicwindow.com/islamic-books/"],
  ["04-blog", "https://arabicwindow.com/blog/"],
  ["05-about-us", "https://arabicwindow.com/about-us/"],
  ["06-why-us", "https://arabicwindow.com/why-us/"],
  ["07-pricing", "https://arabicwindow.com/pricing/"],
  ["08-contact-us", "https://arabicwindow.com/contact-us/"],
  ["09-register", "https://arabicwindow.com/register/"],
  ["10-checkout", "https://arabicwindow.com/checkout/"]
];

fs.mkdirSync(outputDirectory, { recursive: true });

for (const file of fs.readdirSync(outputDirectory)) {
  if (/^\d{2}-.*\.jpe?g$/i.test(file) || file === "manifest.json") {
...
async function preparePage() {
  return evaluate(`(async () => {
    ...
    style.textContent = [
      ...
      ".wpb_animate_when_almost_visible,.wpb_start_animation,.elementor-invisible{opacity:1!important;visibility:visible!important;transform:none!important;animation:none!important}",
      "*,*::before,*::after{transition-duration:.001s!important;caret-color:transparent!important}",
      ".grecaptcha-badge{display:none!important}"
    ].join("");
    document.head.appendChild(style);

    const closeLabels = ["accept", "accept all", "allow all", "got it", "close", "×"];
    for (const element of document.querySelectorAll("button, a")) {
      ...
      if (closeLabels.includes(text) && element.offsetParent !== null) {
...
async function captureTiledJpeg(width, height, destination) {
  const tileDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "arabic-window-tiles-"));
...
try {
  for (let index = 0; index < pages.length; index += 1) {
    ...
    try {
      if (name === "10-checkout") {
        await waitForPage("https://arabicwindow.com/?add-to-cart=17452");
      }
      await waitForPage(url);
      if (name === "03-books" || name === "04-blog") await sleep(5000);
    ...
    `${JSON.stringify(
      {
        ...
        viewportWidth: 1440,
        scope: "Only the public pages linked in the main navigation, plus a populated checkout page. Individual posts and other site pages are excluded.",
        pages: manifest
      },
      null,
      2
