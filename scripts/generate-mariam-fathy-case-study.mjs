...
const projectRoot = "/projects/mariam-fathy-shop/";
const heroSizes = "(max-width: 1024px) 100vw, 1024px";
const heroCandidates = [800, 1200, 1600];
...
function previewPath(target) {
  return `${projectRoot}previews/${target.replace(/\.[^.]+$/, ".webp")}`;
}

function heroSrcset(extension) {
  return heroCandidates.map((width) => `${projectRoot}optimized/hero-${width}.${extension} ${width}w`).join(", ");
}

function heroPreload() {
  return `<link rel="preload" as="image" type="image/avif" imagesrcset="${heroSrcset("avif")}" imagesizes="${heroSizes}"/>`;
}

function heroMarkup(locale) {
  const title = escapeHtml(content[locale].title);
  return `<picture class="absolute inset-0 block"><source type="image/avif" srcset="${heroSrcset("avif")}" sizes="${heroSizes}"/><source type="image/webp" srcset="${heroSrcset("webp")}" sizes="${heroSizes}"/><img alt="${title} storefront cover" src="${projectRoot}optimized/hero-1200.webp" srcset="${heroSrcset("webp")}" sizes="${heroSizes}" width="1200" height="675" loading="eager" fetchpriority="high" decoding="async" class="object-cover object-top" style="position:absolute;height:100%;width:100%;left:0;top:0;right:0;bottom:0;color:transparent"/></picture>`;
}
...
function screenshotCard(locale, group, item) {
  ...
  const imagePath = `${projectRoot}${item.target}`;
  const displayPath = previewPath(item.target);
  return `<div class="flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-[#111827] shadow-lg hover:border-white/20 hover:shadow-[0_8px_30px_rgba(56,189,248,0.07)] transition-all duration-300" style="${maxWidth}"><button type="button" aria-label="${escapeHtml(caption)} — ${ariaSuffix}" data-case-study-screenshot data-full-src="${imagePath}" data-gallery-group="${group.id}" class="relative block w-full text-left rtl:text-right outline-none focus-visible:ring-2 focus-visible:ring-[#38BDF8] focus-visible:ring-inset cursor-zoom-in" style="width:100%;height:${height};overflow-x:hidden;overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;touch-action:pan-y;scrollbar-width:thin;scrollbar-color:rgba(148,163,184,.8) rgba(15,23,42,.65);flex-shrink:0"><img src="${displayPath}" alt="${escapeHtml(`Mariam Fathy Shop — ${caption}`)}" loading="lazy" decoding="async" style="position:static;width:100%;height:auto;max-width:none;display:block;transform:translateY(0);transition:none;will-change:auto"/><div aria-hidden="true" style="position:sticky;bottom:0;left:0;right:0;height:80px;background:linear-gradient(to top, rgba(17,24,39,0.95) 0%, transparent 100%);opacity:1;transition:opacity 0.5s ease;pointer-events:none"></div><div aria-hidden="true" class="absolute top-3 right-3 flex items-center gap-1 px-2.5 py-1.5 bg-black/60 border border-white/15 text-white/70 text-xs font-semibold rounded-lg backdrop-blur-sm">${locale === "ar" ? "صفحة كاملة" : "Full page"}</div><div aria-hidden="true" style="position:sticky;bottom:12px;left:50%;transform:translateX(-50%);opacity:0.85;transition:opacity 0.3s ease;white-space:nowrap;pointer-events:none;width:max-content;margin-left:auto;margin-right:auto" class="flex items-center gap-1.5 px-3 py-1.5 bg-black/70 backdrop-blur-sm border border-white/10 rounded-full"><span class="text-white/70 text-xs font-medium">${locale === "ar" ? "مرّر داخل الإطار لعرض الصورة كاملة" : "Scroll inside frame to view full screenshot"}</span></div><div aria-hidden="true" class="absolute inset-0 flex items-center justify-center bg-black/0 hover:bg-black/20 transition-colors duration-300 pointer-events-none"><div style="opacity:0;transform:scale(0.8);transition:opacity 0.2s ease, transform 0.2s ease" class="p-3 bg-black/60 backdrop-blur-sm border border-white/10 rounded-full"><svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v6m3-3H7"></path></svg></div></div></button><div class="px-5 py-4 space-y-1.5 flex-1"><p class="text-white font-semibold text-base leading-snug">${escapeHtml(caption)}</p></div></div>`;
}
...
for (const [locale, template] of Object.entries(templates)) {
  ...
  html = html.replace(/<link rel="preload"[^>]*href="\/projects\/mariam-fathy-shop\/cover\.png"[^>]*\/>/, heroPreload());
